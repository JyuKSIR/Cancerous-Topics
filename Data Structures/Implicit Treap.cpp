...
/*
===============================
IMPLICIT TREAP (THIS CODE)
===============================

TYPE:
- Randomized Balanced BST (Treap)
- Works like a dynamic array (index-based)
...
erase(l, r)             -> delete subarray [l..r]
get_val(pos)            -> get value at index
size()                  -> current size
...
2) RANGE QUERY
--------------
query(l, r)             -> sum of range [l..r]

TIME: O(log n)

-------------------

3) RANGE UPDATES (LAZY)
-----------------------
update(l, r, x)         -> add x to all elements in range
replace(l, r, x)        -> set all elements in range to x
reverse(l, r)           -> reverse subarray
cyclic_shift(l, r, k)   -> rotate subarray left/right
...
4) SEARCH
---------
get_pos(value)          -> index of value (only reliable if values are unique)
find(value)             -> check if value exists
...
5) DEBUG / OUTPUT
-----------------
print_array()           -> print whole array in order
...
✔ Range query + range update problems
✔ Subarray reversal problems
✔ Rotation / cyclic shift problems
✔ Text editor / sequence editor simulation
✔ Competitive programming sequence manipulation
✔ Problems needing split + merge on arrays

Examples:
- reverse subarray queries
- add value to range
- range assignment
- maintain array with insert/delete anywhere
- playlist rotation
- DNA / string sequence editing (as array)
...
Range Add            | O(log n)
Range Replace        | O(log n)
Reverse              | O(log n)
Cyclic Shift         | O(log n)
Get Value            | O(log n)
Find Position        | O(log n) expected
...
- Duplicate values break get_pos/find logic
- position map is not updated after replace/update
- erase does not free memory (leak risk)
- find-by-value is unreliable in general
